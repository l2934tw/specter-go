...
type WelcomeConfig struct {
	...
	JoinImageURL     *string // Preferred remote image URL.
	JoinDMEnabled    bool
...
func (s *Store) GetWelcomeConfig(ctx context.Context, guildID string) (*WelcomeConfig, error) {
	row := s.pool.QueryRow(ctx, `
		SELECT guild_id, join_enabled, join_channel_id, join_message, join_image_enabled, join_image_path, join_image_url,
	...
	var c WelcomeConfig
	err := row.Scan(&c.GuildID, &c.JoinEnabled, &c.JoinChannelID, &c.JoinMessage, &c.JoinImageEnabled, &c.JoinImagePath, &c.JoinImageURL,
...
func (s *Store) UpsertWelcomeConfig(ctx context.Context, c *WelcomeConfig) error {
	_, err := s.pool.Exec(ctx, `
		INSERT INTO welcome_config (guild_id, join_enabled, join_channel_id, join_message, join_image_enabled, join_image_path, join_image_url,
		...
		c.GuildID, c.JoinEnabled, c.JoinChannelID, c.JoinMessage, c.JoinImageEnabled, c.JoinImagePath, c.JoinImageURL,
