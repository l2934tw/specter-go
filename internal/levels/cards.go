...
func RenderWelcomeCard(ctx context.Context, d WelcomeCardData) ([]byte, error) {
	...
	if bg := fetchBackground(ctx, d.BackgroundURL); bg != nil {
		...
		if zoom != 100 {
			covered = resizeImage(covered, int(float64(w)*float64(zoom)/100), int(float64(h)*float64(zoom)/100))
		}
		b := covered.Bounds()
...
func RenderLevelUpCard(ctx context.Context, d LevelUpCardData) ([]byte, error) {
	...
	if err := dc.LoadFontFace(findFont(), 18); err == nil {
		...
		dc.DrawString(fmt.Sprintf("Rank #%d    •    %d XP", d.Rank, d.XP), 365, 195)
	}
