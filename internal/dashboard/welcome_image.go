...
func (s *Server) handleWelcomeImageUpload(w http.ResponseWriter, r *http.Request) {
	...
	defer file.Close()
	if header.Size > maxWelcomeImageSize {
		http.Error(w, "image must be 10 MB or smaller", http.StatusBadRequest)
		return
	}

	data, err := io.ReadAll(io.LimitReader(file, maxWelcomeImageSize+1))
	if err != nil || len(data) > maxWelcomeImageSize {
	...
	if err != nil {
		http.Error(w, "unsupported image: use PNG, JPEG or GIF", http.StatusBadRequest)
		return
	}
	path := filepath.Join("data", "welcome", gid+"."+format)
	if err := os.MkdirAll(filepath.Dir(path), 0o755); err != nil {
		http.Error(w, "failed to prepare image storage", http.StatusInternalServerError)
		return
	}
	...
	if err := s.store.UpsertWelcomeConfig(r.Context(), cfg); err != nil {
		_ = os.Remove(path)
		http.Error(w, "failed to save welcome settings", http.StatusInternalServerError)
		return
	}
...
func (s *Server) handleWelcomeImage(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil || cfg.JoinImagePath == nil || *cfg.JoinImagePath == "" {
		http.NotFound(w, r)
		return
	}
	path := *cfg.JoinImagePath
	if filepath.Base(path) != gid+filepath.Ext(path) || filepath.Dir(filepath.Clean(path)) != filepath.Join("data", "welcome") {
		http.NotFound(w, r)
		return
	}
...
func validateWelcomeImage(data []byte) (string, error) {
	...
	case "png", "jpeg", "gif":
		return format, nil
	default:
		return "", errUnsupportedWelcomeImage{}
	}
}

type errUnsupportedWelcomeImage struct{}
func (errUnsupportedWelcomeImage) Error() string { return "unsupported image format" }

func removeWelcomeImageFile(path *string, keep string) {
	if path == nil || *path == "" || *path == keep {
		return
	}
	_ = os.Remove(*path)
}
